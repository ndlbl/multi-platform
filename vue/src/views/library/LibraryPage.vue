...
<template>
  <section class="mx-auto max-w-4xl space-y-6 p-4">
    ...
    <div
      v-if="libraryQuery.isError.value"
    ...
    <ul
      v-if="libraryQuery.isPending.value"
    ...
    <ul
      ...
      class="divide-y divide-slate-200 rounded-lg border border-slate-200 bg-white"
    >
      <li v-if="filteredItems.length === 0" class="p-6 text-center text-slate-500">
      ...
      >
        <input
          ...
          @change="toggleMutation.mutate({ id: item.id, consumed: item.consumed })"
        />
        <div class="flex-1">
          <div class="flex items-baseline justify-between gap-2">
            <label :for="'check-' + item.id">
              <h3
                class="font-medium"
                :class="item.consumed ? 'text-slate-300 line-through' : 'text-slate-900'"
              >
                {{ item.title }}
              </h3>
            </label>
            <span class="text-sm text-slate-500">{{ item.kind }}</span>
          </div>

          <p class="mt-1 text-sm text-slate-600">
            <template v-if="item.kind === 'book'">
              {{ item.author }} · {{ item.pages }} pages
            </template>
            <template v-else-if="item.kind === 'podcast'">
          ...
          <div v-if="item.tags?.length" class="mt-2 flex flex-wrap py-1">
            <span
              ...
              :key="tag"
              class="mr-2 rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-600"
            >
              #{{ tag }}
            </span>
          </div>
