...
describe('Angular · Registration flow', () => {
  const REGISTER_EMAIL = `e2e-ng-register@${Cypress.env('testEmailDomain')}`
  const VERIFY_EMAIL = `e2e-ng-verify@${Cypress.env('testEmailDomain')}`
  ...
  it('submits the registration form and reaches the email-verification stage', () => {
    cy.visit('/register')
    cy.get('input[formControlName="email"]').type(REGISTER_EMAIL)
    ...
    cy.get('input[formControlName="email"]').should('have.value', REGISTER_EMAIL)
  })
  ...
  it('completes verification with a seeded OTP and logs the user in', () => {
    ...
    cy.get('input[formControlName="email"]').should('have.value', VERIFY_EMAIL)
    cy.get('input[formControlName="code"]').type('TEST01')
    cy.get('button[type="submit"]').click()
