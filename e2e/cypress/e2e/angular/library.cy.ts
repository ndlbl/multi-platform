...
describe('Angular · Library', () => {
  ...
  it('adds a book through the dialog and shows it in the list', () => {
    ...
    cy.get('#add-item-dialog').within(() => {
      cy.get('select[formControlName="kind"]').select('book')
      cy.get('input[formControlName="title"]').type(title)
      cy.get('input[formControlName="author"]').type('Robert C. Martin')
      cy.get('input[formControlName="pages"]').clear().type('464')
